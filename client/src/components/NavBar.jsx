...
const categoriesArray = data || [];
    ...
    return categories?.map((category, idx) => {
      ...
      if (typeof category === "string") {    
        return (
          ...
          <li className="  rounded-lg" key={idx}>
            <a className=" hover:bg-sky-300 " href={category?.url}>{category} </a>
          </li>
        );
      } else {
        ...
        return (
          <li
            key={idx}
            className="group/category rounded-lg  relative"
          >
            <a className=" hover:bg-sky-300 " href={category?.url}>{category?.category}</a>
       {category.subcategories[0]&&(     <ul
            ...
            >
              {category?.subcategories?.map((subCat, subIdx) => (
                <li className="hover:bg-sky-200 rounded-lg" key={subIdx}>
                  <a className=" hover:bg-sky-300 " href={subCat?.url}>{subCat?.category}</a>
                </li>
              ))}
            </ul>)}
          </li>
  ...
  return (
    ...
    <div
      ref={navRef}
      className="navbar z-50 2xl:px-40 2xl:py-2 p-0 bg-base-100 ">
      <div className="navbar-start z-50">
        ...
        <a href="/" className="cursor-pointer min-h-8 h-full duration-200 transition-all">
          <img
        ...
        <ul className="menu menu-horizontal  h-full px-1">
          { categoriesArray?.map((section, index) => (
            ...
            >
              <a
                href="#"
                className="transition-all border-b-4 border-transparent hover:bg-sky-300 hover:border-b-4 hover:border-sky-700  duration-100 rounded-none font-medium"
              >
                {section?.section}
              </a>
              <ul
              ...
              >
                {section?.subsections?.map((subsection, subIndex) => (
                  <li
                    key={subIndex}
                    className="group/subsection rounded-lg relative"
                  >
                    <a className=" hover:bg-sky-300 " href="#">{subsection?.title}</a>
                    <ul
      ...
      <div className="navbar-end z-10">
        <a className="btn hover:bg-sky-200  hover:text-black 500 btn-outline btn-xs md:btn-md 2xl:btn-md backdrop-blur-sm shadow-xl">
          Member Portal
        </a>
      </div>
    </div>
